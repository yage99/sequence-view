...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniCropBitmap(
		JNIEnv * env, jobject obj, jobject handle, uint16_t left, uint16_t top,
		uint16_t right, uint16_t bottom);
//scale using nearest neighbor
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniScaleNNBitmap(
		JNIEnv * env, jobject obj, jobject handle, uint16_t newWidth,
		uint16_t newHeight);
...
void convertIntToArgb(uint16_t pixel, ARGB* argb) {
...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniCropBitmap(
	...
	uint16_t oldWidth = jniBitmap->_bitmapInfo.width;
	uint16_t newWidth = right - left, newHeight = bottom - top;
	uint16_t* newBitmapPixels = new uint16_t[newWidth * newHeight];
	uint16_t* whereToGet = previousData + left + top * oldWidth;
	uint16_t* whereToPut = newBitmapPixels;
	for (int y = top; y < bottom; ++y) {
		memcpy(whereToPut, whereToGet, sizeof(uint16_t) * newWidth);
		whereToGet += oldWidth;
		whereToPut += newWidth;
	}
...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniRotateBitmapCcw90(
	...
	if (jniBitmap->_storedBitmapPixels == NULL)
		return;
	uint16_t* previousData = jniBitmap->_storedBitmapPixels;
	uint16_t newWidth = jniBitmap->_bitmapInfo.height;
	uint16_t newHeight = jniBitmap->_bitmapInfo.width;
	jniBitmap->_bitmapInfo.width = newWidth;
	jniBitmap->_bitmapInfo.height = newHeight;
	uint16_t* newBitmapPixels = new uint16_t[newWidth * newHeight];
	int whereToGet = 0;
...
// ... X.. .YX ...
	for (int x = 0; x < newWidth; ++x)
		for (int y = newHeight - 1; y >= 0; --y) {
			...
			uint16_t pixel = previousData[whereToGet++];
...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniRotateBitmap180(
	...
	uint16_t width = jniBitmap->_bitmapInfo.width;
	uint16_t height = jniBitmap->_bitmapInfo.height;
...
// cdef 4321
	...
	for (int y = height - 1; y >= height / 2; --y)
		for (int x = width - 1; x >= 0; --x) {
			//take from each row (up to bottom), from left to right
			uint16_t tempPixel = pixels2[width * y + x];
...
//
	...
	int pixelsCount = jniBitmap->_bitmapInfo.height
			* jniBitmap->_bitmapInfo.width * jniBitmap->multi;
...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniScaleNNBitmap(
	...
	if (jniBitmap->_storedBitmapPixels == NULL)
		return;
	uint16_t oldWidth = jniBitmap->_bitmapInfo.width;
	uint16_t oldHeight = jniBitmap->_bitmapInfo.height;
	uint16_t* previousData = jniBitmap->_storedBitmapPixels;
	uint16_t* newBitmapPixels = new uint16_t[newWidth * newHeight];
	int x2, y2;
	int whereToPut = 0;
